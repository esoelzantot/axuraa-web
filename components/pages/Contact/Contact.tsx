// components/pages/Contact/Contact.tsx
"use client";
...
import { ContactEmail, SocialLink } from "@/service/Contact/contactinformation";
...
// ── Fallback data ──────────────────────────────────────────────────────────
const FALLBACK_PLATFORMS: SocialMediaPlatform[] = [
  { name: "Facebook", link: "#", icon: FacebookIcon },
  { name: "Instagram", link: "#", icon: InstagramIcon },
  { name: "LinkedIn", link: "#", icon: LinkedInIcon },
  { name: "TikTok", link: "#", icon: TikTokIcon },
];

// ── Component ──────────────────────────────────────────────────────────────
const Contact = ({ contactData }: Props) => {
  // Transform social links → SocialMediaPlatform[]
  const socialPlatforms: SocialMediaPlatform[] = contactData?.socialLinks
    ?.length
    ? contactData.socialLinks.map((link: SocialLink) => ({
        name: link.name,
        link: link.url,
        icon: ICON_MAP[link.name] ?? FacebookIcon,
      }))
    : FALLBACK_PLATFORMS;
  ...
  // Transform email contacts
  const contactsData =
    contactData?.emails?.map((email: ContactEmail) => ({
      name: email.name,
      email: email.email,
  ...
  return (
    <div id="contact-section" className={styles.container}>
      <HeroSection
        ...
        subtitle1="Have a project in mind? We'd love to hear about it."
        subtitle2=""
